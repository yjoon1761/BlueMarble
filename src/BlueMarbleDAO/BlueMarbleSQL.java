...
public class BlueMarbleSQL {
    ...
    public String getLocationName(int newPosition) {
        // 게임 보드의 위치 정보를 데이터베이스 또는 다른 데이터 소스에서 가져와서 locations 배열 초기화
        String[] locations = {"출발", "서울", "인천", "대전", "전주", "동두천", "홍천", "영종도", "남원", "무인도", "울릉도", "의정부", "한번더", "횡성", "영주시", "남해", "제주도", "양양", "남양주", "부산", "강화도", "강릉", "춘천", "파주"};

        // newPosition이 유효한 범위인지 확인
        if (newPosition >= 0 && newPosition < locations.length) {
            return locations[newPosition];
        } else {
            return "알 수 없는 위치";
        }
    }
